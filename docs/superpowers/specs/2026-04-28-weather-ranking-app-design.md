...
The backend uses Hono as the HTTP application and GraphQL Yoga at `/graphql`. GraphQL resolvers stay thin and delegate to `ranking.service.ts`. The service orchestrates city lookup, weather lookup, and pure ranking logic. Ranking code lives in `domain/ranking` and does not depend on Hono, GraphQL, or Open-Meteo response shapes.

The frontend uses Vite, React, and urql. It owns UI state and GraphQL queries only. It does not import backend service or resolver types. Frontend operation types are generated from the shared SDL contract in `packages/contracts/schema.graphql`.
...
8. `bun run generate` regenerates frontend query types when the API contract or operation documents change.
